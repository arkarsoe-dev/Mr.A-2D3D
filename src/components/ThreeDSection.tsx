...
export const ThreeDSection: React.FC<ThreeDSectionProps> = ({ items, loading, numeralMode }) => {
  ...
  return (
    <div className="space-y-5">
      {/* Latest 3D Winning Number Hero Card */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-950/60 via-slate-900 to-slate-950 border-2 border-amber-500/40 p-6 sm:p-8 shadow-2xl shadow-amber-950/40">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-left space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30">
              ...
              <span>ထိုင်း ၃လုံးထီ နောက်ဆုံးထွက်ဂဏန်း (Latest 3D)</span>
            </div>

            <h2 className="text-xl sm:text-2xl font-black text-slate-100 flex items-center justify-center md:justify-start gap-2">
          ...
          {latestDraw ? (
            <div className="flex items-center gap-3 sm:gap-4">
              {latestDraw.result.split('').map((digit, idx) => (
                <div
                  key={idx}
                  className="w-16 h-20 sm:w-20 sm:h-24 rounded-2xl bg-gradient-to-b from-amber-300 via-amber-500 to-amber-700 p-0.5 shadow-xl shadow-amber-500/20"
                >
                  <div className="w-full h-full rounded-[14px] bg-slate-950 flex flex-col items-center justify-center border border-amber-300/30">
                    <span className="font-num text-4xl sm:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-100 via-yellow-300 to-amber-500 drop-shadow-md">
        ...
        <div className="mt-6 pt-4 border-t border-slate-800 flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <span>နောက်တစ်ကြိမ် ၃လုံးထီ ထွက်မည့်ရက်:</span>
          ...
          >
            <span>မူရင်း စာမျက်နှာ ကြည့်ရန်</span>
      ...
      <div className="rounded-2xl bg-slate-900/80 border border-slate-800 p-4 sm:p-6 shadow-xl">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-slate-100">
              ယခင် ၃လုံးထီ ထီပေါက်စဉ် မှတ်တမ်းများ (History Records)
            </h3>
          </div>
        ...
        <div className="mt-5 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
          <span>စုစုပေါင်း ရလဒ်: {formatNumeral(filteredItems.length, numeralMode)} ကြိမ်</span>
          <span className="text-[11px] text-emerald-400 flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-amber-400" />
            တရားဝင် ထိုင်းအစိုးရ ထီပေါက်စဉ် အခြေခံ
          </span>
        </div>
      </div>
    </div>
